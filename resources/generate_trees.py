...
TREES: dict[str, Tree] = {
    'crimson': Tree('aspen', None, None),
    'warped': Tree('jungle', None, None)
}
...
def create_log_block_tag(wood: str, block_name: str, block_axis: StringTag, adj: Pos) -> CompoundTag:
    ...
    if block_name == 'minecraft:oak_log':
        block_name = StringTag('beneath:wood/log/%s' % wood)
    else:
        block_name = StringTag('beneath:wood/wood/%s' % wood)
...
def create_leaf_block_tag(wood: str, distance: int) -> CompoundTag:
    ...
    block = CompoundTag()
    block['Name'] = StringTag('beneath:wood/leaves/%s' % wood)
    block['Properties'] = CompoundTag()
...
def save_structure(path: str, root_nbt: RootTag):
    ...
    # root_nbt.save('./out/production/resources/data/beneath/structures/' + path.replace('/', '_'), gzipped=True)
