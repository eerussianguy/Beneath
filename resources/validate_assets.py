...
def main():
    errors = 0
    model_locations = glob(ASSETS_PATH + 'beneath/models/**/*.json', recursive=True)
    state_locations = glob(ASSETS_PATH + 'beneath/blockstates/**/*.json', recursive=True)
...
def validate_lang(state_locations, lang_json, sound_json):
    ...
    for f in state_locations:
        name = f.replace('\\', '/')
        name = name.replace(ASSETS_PATH + 'beneath/blockstates/', '')
        name = name.replace('.json', '')
        name = name.replace('/', '.')
        if 'block.beneath.%s' % name not in lang_json and 'plant' not in name:
    ...
    #    else:
    #        sub = data['subtitle']
    #        if 'beneath' in sub and sub not in lang_json:
...
def find_model_file(file_path: str, initial_path: str, tested: int, errors: int, on_error: str):
    ...
    if res.domain == 'beneath':
        tested += 1
        path = ASSETS_PATH + 'beneath/models/%s.json' % res.path
        found = len(glob(path))
        if found != 1:
